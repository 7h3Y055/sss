...
/*   Updated: 2024/01/19 12:03:21 by ybouchma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*clean(char *buffer)
{
	...
	if (ft_strlen_copy(buffer) <= 0 || !ft_strchr_copy(buffer, '\n'))
		return (free(buffer), NULL);
	i = 0;
	...
	buf = ft_calloc_copy(sizeof(char), ft_strlen_copy(buffer + i));
	if (!buf || !ft_strlen_copy(buffer + i))
	{
		free(buf);
		return (NULL);
	}
...
char	*get_nline(char *buffer)
{
	...
	while (buffer[i] && buffer[i] != '\n')
		i++;
	line = ft_calloc_copy(sizeof(char), i + 2);
	i = 0;
	while (buffer[i])
	{
		line[i] = buffer[i];
...
char	*read_data(int fd, char *buffer)
{
	...
	data = ft_calloc_copy(sizeof(char), BUFFER_SIZE + 1);
	if (!data)
		return (NULL);
	r = 1;
	while (r > 0)
...
char	*merge(char *s1, char *s2)
{
	...
	str = (char *)malloc(sizeof(char) * (ft_strlen_copy(s1) + ft_strlen_copy(s2) + 1));
	if (str == NULL)
